...
const swapUsingUniversalRouter = async (
  ...
  const { signature: permitSignature, permit } = await approvePermit(
    sdk,
    WMATIC.address,
    ethers.constants.MaxUint256.toString(),
    PERMIT2_ADDRESS,
  ...
  console.log(
    ...
    WMATIC_USDC.token0.symbol,
    await balance(sdk, WMATIC_USDC.token0.address),
  );
  console.log(
    "Smart Wallet Balance for Token",
    WMATIC_USDC.token1.symbol,
...
const _mintWMatic = async (sdk: ThirdwebSDK, amount: string) => {
  ...
  console.log(
    `Deposited ${amount} WMATIC from ${await sdk.wallet.getAddress()}`,
  );
};
